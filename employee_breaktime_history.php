...
$where_clauses = ["sb.status = 'returned'", "sb.emp_id = '$current_employee_id'"];
$where_clauses[] = "DATE(sb.break_start) BETWEEN '$start_date' AND '$end_date'";

$where_sql = implode(' AND ', $where_clauses);

$history_sql = "
    SELECT 
        sb.id, sb.period, sb.break_start, sb.break_end, sb.duration_min,
        sb.category, sb.is_edited
...
// STATS
$today_total = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as cnt FROM short_breaks WHERE status = 'returned' AND emp_id = '$current_employee_id' AND DATE(break_start) = CURDATE()"))['cnt'];
$avg_result = mysqli_query($conn, "SELECT AVG(duration_min) as avg_min FROM short_breaks WHERE status = 'returned' AND emp_id = '$current_employee_id' AND DATE(break_start) = CURDATE() AND break_end IS NOT NULL");
$avg_row = mysqli_fetch_assoc($avg_result);
$avg_duration = $avg_row['avg_min'] ? round($avg_row['avg_min']) : 0;
// We'll count violations separately later or keep it generic
$overtime_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as cnt FROM short_breaks WHERE status = 'returned' AND emp_id = '$current_employee_id' AND duration_min > 30 AND DATE(break_start) = CURDATE()"))['cnt'];

if (isset($_GET['export_csv'])) {
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename=my_breaktime_history_' . $start_date . '_to_' . $end_date . '.csv');
    $output = fopen('php://output', 'w');
    fputcsv($output, array('Date', 'Break Out Time', 'Break In Time', 'Duration (min)', 'Status'));
    
    if ($history_result && mysqli_num_rows($history_result) > 0) {
        mysqli_data_seek($history_result, 0);
        while ($row = mysqli_fetch_assoc($history_result)) {
            $duration = $row['duration_min'] ?? 0;
            $is_overtime = $duration > 30;
            $status = $is_overtime ? 'Overtime' : 'Within Limit';
            
            fputcsv($output, array(
                date('M d, Y', strtotime($row['break_start'])),
...
<head>
    ...
    <title>WeRide - My Breaktime History</title>
    <style>
...
<body>
    ...
    <main class="main-content">
        <header class="topbar">
            <div class="page-title">My Breaktime History</div>
        </header>
        ...
        <div class="content">
            <div class="stats-grid">
                <div class="premium-stat-card">
                    ...
                    <div class="info-area">
                        <div class="info-title">Breaks Today</div>
                        <div class="info-value"><?php echo $today_total; ?></div>
                        <div style="font-size: 12px; color: #94a3b8; margin-top: 4px;">Completed breaks</div>
                    </div>
                </div>
                <div class="premium-stat-card">
                    ...
                    <div class="info-area">
                        <div class="info-title">Avg Duration</div>
                        <div class="info-value"><?php echo $avg_duration; ?><span style="font-size: 14px; color: #94a3b8; font-weight: normal;"> min</span></div>
                        <div style="font-size: 12px; color: #94a3b8; margin-top: 4px;">Today's average</div>
                    </div>
                </div>
                <div class="premium-stat-card">
                    ...
                    <div class="info-area">
                        <div class="info-title">Overtime Breaks</div>
                        <div class="info-value" style="color: <?php echo $overtime_count > 0 ? '#ef4444' : 'inherit'; ?>;"><?php echo $overtime_count; ?></div>
                        <div style="font-size: 12px; color: #94a3b8; margin-top: 4px;">Over 30 minutes today</div>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        if ($history_result && mysqli_num_rows($history_result) > 0) {
                            while ($row = mysqli_fetch_assoc($history_result)) {
                                ...
                                $category_display = $row['category'] ? htmlspecialchars($row['category']) : 'Short Break';
                                if (isset($row['is_edited']) && $row['is_edited']) {
                                    $category_display .= " <span title='Edited' style='font-size: 10px; color: #94a3b8;'>(Edited)</span>";
                                }

                                echo "<tr>";
                                echo "<td style='color: #0f172a; font-weight: 600;'>" . date('M d, Y', strtotime($row['break_start'])) . "</td>";
                                echo "<td style='font-weight: 500;'>" . date('h:i A', strtotime($row['break_start'])) . "</td>";
                                echo "<td style='font-weight: 500;'>" . ($row['break_end'] ? date('h:i A', strtotime($row['break_end'])) : '--') . "</td>";
                                $duration_style = $is_overtime ? "color: #ef4444; font-weight: 700;" : "color: #059669; font-weight: 600;";
                                echo "<td style='$duration_style'>" . $duration . " min" . ($is_overtime ? " ⚠️" : "") . "</td>";
                                echo "<td><span class='badge " . ($is_overtime ? "badge-overtime" : "badge-ok") . "'>" . ($is_overtime ? "Overtime" : "Within Limit") . "</span></td>";
                            ...
                            echo "<tr><td colspan='6'>
                                    <div class='empty-state'>
                                        ...
                                        <div class='empty-state-text'>No break records found. Try adjusting your filters.</div>
                                    </div>
                                  </td></tr>";
                        }
                        ?>
