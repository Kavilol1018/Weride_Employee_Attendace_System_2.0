...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save') {
    if (isset($_SESSION['temp_upload_path']) && file_exists($_SESSION['temp_upload_path'])) {
        ...
        $custom_name = isset($_POST['custom_filename']) ? preg_replace('/[^a-zA-Z0-9_\- ]/', '', trim($_POST['custom_filename'])) : '';
        if (empty($custom_name)) {
            $new_filename = "Attendance_" . $timestamp . ".xlsx";
        } else {
            $new_filename = str_replace(' ', '_', $custom_name) . ".xlsx";
        }
        $dest_path = "uploads/" . $new_filename;
        if (copy($_SESSION['temp_upload_path'], $dest_path)) {
            $display_name = str_replace('_', ' ', pathinfo($new_filename, PATHINFO_FILENAME));
            $success = "File saved successfully as $display_name";
            $current_file = $dest_path;
            $_SESSION['current_loaded_file'] = $current_file;
        } else {
            $error = "Failed to save file.";
        }
    } else {
        $error = "No active upload to save.";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        if ($ext === 'xlsx') {
            ...
            $temp_dest = "uploads/temp_upload.xlsx";
            if(move_uploaded_file($file['tmp_name'], $temp_dest)) {
                $_SESSION['temp_upload_path'] = $temp_dest;
                $_SESSION['current_loaded_file'] = "New Upload (Unsaved)";
            } else {
...
if (isset($_SESSION['temp_upload_path']) && file_exists($_SESSION['temp_upload_path'])) {
    if ($xlsx = SimpleXLSX::parse($_SESSION['temp_upload_path'])) {
        $isFirst = true;
        $colMap = [];
        $emptyCount = 0;
        foreach ($xlsx->readRows() as $row) {
            ...
            if (empty($check_out) || strtolower($check_out) === 'not checked out' || $check_out === 'Not Checked Out') {
                ...
                if (!empty($shift_end)) {
                    if (strtotime($check_out) < strtotime($shift_end)) {
                        $checkout_status = 'Emergency Leave';
                    } else {
            ...
            } elseif ($status === 'Late' || $checkout_status === 'Emergency Leave' || $checkout_status === 'Missed') {
...
$current_page = 'Attendance System'; // for sidebar logic
...
<head>
    ...
    <title>WeRide - Attendance System</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, sans-serif; }
        ...
        .upload-area { flex: 1; min-width: 300px; border: 2px dashed #cbd5e1; border-radius: 12px; padding: 20px; text-align: center; background: #f8fafc; transition: 0.3s; position: relative; cursor: pointer;}
        ...
        .btn-save { background: #1e293b; color: white; border: none; padding: 12px 30px; border-radius: 10px; font-weight: 700; cursor: pointer; transition: 0.2s; font-size: 14px; margin-top: 20px; }
        ...
        .success-msg { background: #ecfdf5; color: #065f46; padding: 12px; border-radius: 8px; margin-bottom: 20px; font-weight: 600; border: 1px solid #a7f3d0; }
...
<body>
    ...
    <main class="main-content">
        <header class="topbar">
            <div class="page-title">Attendance System</div>
        </header>
        ...
        <div class="content">
            ...
            <div class="controls-wrapper">
                <!-- Upload Area -->
                <form action="" method="POST" enctype="multipart/form-data" style="flex: 1;">
                    <div class="upload-area">
                        <div style="font-size: 24px; margin-bottom: 8px;">📄</div>
                        <h4 style="color: #334155; margin-bottom: 4px;">Upload New Attendance File</h4>
                        <p style="font-size: 12px; color: #94a3b8;">Drag & drop .xlsx or click here</p>
                        <input type="file" name="file" accept=".xlsx" required onchange="this.form.submit()">
                    </div>
                </form>
            ...
            <div class="report-section">
                    ...
                    <div class="filter-buttons" style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                        ...
                        <div class="export-dropdown" style="position: relative; margin-left: auto;">
                            ...
                            <div id="exportMenu" class="dropdown-content" style="display: none; position: absolute; right: 0; top: 100%; background: #ffffff; min-width: 150px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.1); z-index: 10; border-radius: 8px; border: 1px solid #e2e8f0; margin-top: 5px; overflow: hidden;">
                                <a href="export_attendance.php?source=index&view=<?php echo $view; ?>" style="color: #334155; padding: 12px 16px; text-decoration: none; display: block; font-size: 13px; font-weight: 600; border-bottom: 1px solid #e2e8f0;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">CSV Document</a>
                    ...
                    <table class="report-table">
                        ...
                        <tbody>
                            ...
                            foreach ($filtered_data as $row): 
                                ...
                                $outBadge = 'badge-ok';
                                if ($row['checkout_status'] === 'Emergency Leave') $outBadge = 'badge-late';
                ...
                <?php if(isset($_SESSION['current_loaded_file']) && $_SESSION['current_loaded_file'] === 'New Upload (Unsaved)'): ?>
                <div style="text-align: right; margin-top: 20px;">
                    <form method="POST" action="" style="display: flex; justify-content: flex-end; align-items: center; gap: 10px;">
                        <input type="hidden" name="action" value="save">
                        <input type="text" name="custom_filename" placeholder="Enter file name (optional)" style="padding: 10px 14px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 14px; width: 250px; outline: none; transition: 0.2s;" class="form-input">
