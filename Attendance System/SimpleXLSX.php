...
//            $m1 = memory_get_usage();
            ...
            if (\PHP_VERSION_ID < 80000 && LIBXML_VERSION < 20900 && function_exists('libxml_disable_entity_loader')) {
                $_old = @call_user_func('libxml_disable_entity_loader', true);
            }
            ...
            if (self::strtoupper($entry['path']) === $dir && self::strtoupper($entry['name']) === $name) {
                ...
                switch ($entry['cm']) {
                    ...
                    case 8: // Deflated
                        ...
                        break;
                    case 12: // BZIP2
                        if (extension_loaded('bz2') && function_exists('bzdecompress')) {
                            $entry['data'] = \call_user_func('bzdecompress', $entry['data']);
                        } else {
                            $entry['error'] = 7;
...
//      $this->error( 'Invalid cell index ' . $cell );
    ...
    public function readRowsEx($worksheetIndex = 0, $limit = 0)
    {
        if (!$this->rowsExReader) {
            ...
            $className = '\Shuchkin\SimpleXLSXEx';
            $this->rowsExReader = new $className($this);
        }
