...
// Fetch employees
$emp_sql = "SELECT e.emp_id, e.name, e.lunch_window FROM employees e WHERE e.role = 'employee' $emp_filter_sql";
...
// Fetch all lunch breaks in range
...
// Fetch all short breaks in range
$sb_sql = "SELECT emp_id, DATE(break_start) as b_date, break_start, break_end, status,
              TIMESTAMPDIFF(MINUTE, break_start, CURRENT_TIMESTAMP) as current_duration,
              TIMESTAMPDIFF(MINUTE, break_start, break_end) as finished_duration
              FROM short_breaks 
              WHERE DATE(break_start) BETWEEN '$start_date' AND '$end_date'";
...
$sb_by_emp_date = [];
while ($row = mysqli_fetch_assoc($sb_result)) {
    $sb_by_emp_date[$row['emp_id']][$row['b_date']][] = $row;
}
...
foreach ($employees as $e) {
    ...
    while ($loop_date <= $end_date_obj) {
        ...
        if (isset($absences[$emp_id][$date_str])) {
            ...
            // Check short breaks first
            if (isset($sb_by_emp_date[$emp_id][$date_str])) {
                foreach ($sb_by_emp_date[$emp_id][$date_str] as $sb) {
                    if ($sb['status'] == 'returned' && $sb['finished_duration'] > 30) {
                        $violation_type = 'Overtime Short Break';
                        $violation_details = $sb['finished_duration'] . ' minutes on ' . date('H:i', strtotime($sb['break_start']));
                        break;
                    } elseif ($sb['status'] == 'on_break' && $sb['current_duration'] > 30) {
                        $violation_type = 'Overtime Short Break (Ongoing)';
                        $violation_details = $sb['current_duration'] . ' minutes so far';
                        break;
                    }
                }
            }
            ...
            if (!$violation_type) {
                if (!isset($breaks_by_emp_date[$emp_id][$date_str])) {
                    // Never started a lunch break on this date
                    if ($is_past) {
...
<head>
    ...
    <title>WeRide - HR Violations</title>
    <style>
