...
$sql = "
    ...
    WHERE e.role = 'employee' AND sb.status = 'returned' $emp_filter_sql
...
$ongoing_sql = "
    ...
    FROM employees e
    JOIN short_breaks sb ON e.emp_id = sb.emp_id AND DATE(sb.break_start) BETWEEN '$start_date' AND '$end_date'
    WHERE e.role = 'employee' AND sb.status = 'on_break' $emp_filter_sql
...
while ($row = mysqli_fetch_assoc($ongoing_result)) {
    ...
    $row['violation_details'] = $row['current_duration'] . ' minutes so far on ' . date('d/m/Y', strtotime($row['b_date']));
    $violations[] = $row;
}
...
<head>
    ...
    <title>WeRide - Admin Violations</title>
    <style>
