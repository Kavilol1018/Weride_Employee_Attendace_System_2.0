...
if (!empty($selected_employees)) {
    ...
    $emp_filter_sql = " AND sb.emp_id IN (" . implode(',', $safe_emps) . ") ";
}
...
// SUMMARY STATS
$total_meals = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as cnt FROM short_breaks sb WHERE sb.status = 'returned' AND DATE(sb.break_start) BETWEEN '$start_date' AND '$end_date' $emp_filter_sql"))['cnt'];

$avg_result = mysqli_query($conn, "SELECT AVG(sb.duration_min) as avg_min FROM short_breaks sb WHERE sb.status = 'returned' AND DATE(sb.break_start) BETWEEN '$start_date' AND '$end_date' AND sb.break_end IS NOT NULL $emp_filter_sql");
$avg_row = mysqli_fetch_assoc($avg_result);
$avg_duration = $avg_row['avg_min'] ? round($avg_row['avg_min']) : 0;

$overtime_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as cnt FROM short_breaks sb WHERE sb.status = 'returned' AND sb.duration_min > 30 AND DATE(sb.break_start) BETWEEN '$start_date' AND '$end_date' $emp_filter_sql"))['cnt'];
...
// FULL REPORT DATA — all completed breaktimes
$report_sql = "
    SELECT 
        sb.id as break_id,
    ...
    WHERE sb.status = 'returned' AND DATE(sb.break_start) BETWEEN '$start_date' AND '$end_date' $emp_filter_sql
    ORDER BY sb.break_start ASC
";
...
if (isset($_GET['export_csv'])) {
    ...
    header('Content-Disposition: attachment; filename=breaktime_report_' . $start_date . '_to_' . $end_date . '.csv');
    $output = fopen('php://output', 'w');
    fputcsv($output, array('ID', 'Name', 'Type', 'Period', 'Break Start Time', 'Break End Time', 'Duration (min)', 'Status'));
...
<head>
    ...
    <title>WeRide - Breaktime History | <?php echo $display_date; ?></title>
    <style>
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="print-header">
            ...
            <div class="print-subtitle">Breaktime Management System</div>
            <div class="print-report-title">Breaktime Report</div>
        ...
        <header class="topbar">
            <div class="page-title">Breaktime Report</div>
        </header>
        ...
        <div class="content">
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-icon bg-blue-50" style=" color: #3b82f6;">☕</div>
                    <div class="stat-title">Total Breaks</div>
                    <div class="stat-value"><?php echo $total_meals; ?></div>
                    <div class="stat-subtitle">Breaktimes taken</div>
                </div>
                <div class="stat-card">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-subtitle">Under 30 minutes</div>
                </div>
                <div class="stat-card">
                    ...
                    <div class="stat-subtitle">Over 30 minutes</div>
                </div>
            </div>
            ...
            <div class="report-section">
                <div class="report-header">
                    <h3 class="report-title">Breaktime Records — <?php echo $display_date; ?></h3>
                ...
                <?php if ($report_result && mysqli_num_rows($report_result) > 0): ?>
                    ...
                    <div class="report-footer">
                        <div class="footer-stat">Total Breaks: <span><?php echo $total_meals; ?></span></div>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <div class="empty-state-text">No breaktime records found for <?php echo $display_date; ?>.</div>
                    </div>
                <?php endif; ?>
            </div>
