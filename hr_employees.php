                    ...
                    <table>
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                echo "<td><span style='background: #fef2f2; padding: 4px 8px; border-radius: 6px; font-size: 12px; font-weight: 700; color: #dc2626;'>" . $absence . " Days</span></td>";
                                
                                echo "<td>
                                        <div class='actions-cell'>
                                            <button type='button' class='btn-edit' style='background-color:#e0e7ff; color:#4338ca;' onclick='openDetailsModal(\"" . htmlspecialchars($row['emp_id'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['name'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['employment_type'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['onboarding_date'] ?? '-', ENT_QUOTES) . "\", " . ($row['annual_leave_balance'] ?? 14) . ", " . ($row['sick_leave_balance'] ?? 14) . ")'>Details</button>
                                            <button type='button' class='btn-edit' onclick='openEditModal(" . $row['id'] . ", \"" . htmlspecialchars($row['emp_id'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['welabel_id'] ?? '', ENT_QUOTES) . "\", \"" . htmlspecialchars($row['name'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['employment_type'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['role'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['lunch_window'], ENT_QUOTES) . "\")'>Edit</button>
