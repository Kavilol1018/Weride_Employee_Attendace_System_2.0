...
$gl_filter_sql = ($view_scope === 'all') ? "" : " AND e.group_leader_id = '$gl_emp_id' ";
$departments = [];
// -------------------------
...
if (isset($_POST['force_end_break'])) {
    ...
    header("Location: gl_live_attendance.php");
    exit();
}
...
$pending_overrides = mysqli_query($conn, "SELECT r.*, e.name FROM break_override_requests r JOIN employees e ON r.emp_id = e.emp_id WHERE r.status = 'pending' $gl_filter_sql");

// STATS QUERIES
$total_employees = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as cnt FROM employees e WHERE 1=1 $gl_filter_sql"))['cnt'];

$on_lunch_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as cnt FROM lunch_breaks lb JOIN employees e ON lb.employee_id = e.emp_id WHERE lb.status = 'on_break' $gl_filter_sql"))['cnt'];
$on_short_break_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as cnt FROM short_breaks sb JOIN employees e ON sb.emp_id = e.emp_id WHERE sb.status = 'on_break' $gl_filter_sql"))['cnt'];
...
$today_breaks = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as cnt FROM lunch_breaks lb JOIN employees e ON lb.employee_id = e.emp_id WHERE lb.status = 'returned' AND DATE(lb.break_start) = CURDATE() $gl_filter_sql"))['cnt'];
?>
...
<head>
    ...
    <title>WeRide - Group Leader Live Attendance</title>
...
<body>
    ...
    <main class="main-content">
        <header class="topbar">
            <div class="page-title">Group Leader Live Attendance</div>
        ...
        <div class="content">
            ...
            <!-- DASHBOARD FILTERS -->
            <details class="bg-white view-options-card" style='border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 16px; box-shadow: 0 2px 4px -1px rgba(0,0,0,0.03); width: max-content;'>
                ...
                <div style="padding: 8px 12px 12px 12px; border-top: 1px solid #e2e8f0; margin-top: 4px;">
                    <div style='display: flex; gap: 15px; flex-wrap: wrap; margin-bottom: 12px; padding-bottom: 12px; border-bottom: 1px solid #e2e8f0;'>
                    ...
                    <div style='display: flex; gap: 15px; flex-wrap: wrap;'>
                        <label style='display: flex; align-items: center; gap: 8px; font-size: 12px; font-weight: 600; color: #475569; cursor: pointer;'>
                            <input type='checkbox' checked onchange='document.getElementById("card-availChart").style.display = this.checked ? "flex" : "none"'> Show Staff Availability
                        </label>
                        <label style='display: flex; align-items: center; gap: 8px; font-size: 12px; font-weight: 600; color: #475569; cursor: pointer;'>
                    ...
                    legend: {
                        ...
                        onClick: function(e, legendItem, legend) {
                            ...
                            chart.data.labels.forEach((label, i) => {
                                // getDataVisibility defaults to true if undefined
