...
if (isset($_POST['add_employee'])) {
    ...
    if (mysqli_num_rows($check) > 0) {
        header("Location: gl_employees.php?error=" . urlencode("Employee ID, WeLabel ID, or Name already exists!"));
        exit();
    }

    $gl_emp_id = $_SESSION['emp_id'];
    $sql = "INSERT INTO employees (emp_id, welabel_id, name, password, employment_type, role, lunch_window, group_leader_id) VALUES ('$emp_id', '$welabel_id', '$name', '$password', '$employment_type', '$role', '$lunch_window', '$gl_emp_id')";
    mysqli_query($conn, $sql);
    
    header("Location: gl_employees.php");
    exit();
}
...
if (isset($_POST['reset_password'])) {
    ...
    header("Location: gl_employees.php?msg=" . urlencode("Password reset to 'password123'"));
    exit();
}
?>
...
<head>
    ...
    <title>WeRide - Group Leader Employees</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, sans-serif; }
        ...
        .dropdown-content button { color: #334155; padding: 12px 16px; text-decoration: none; display: flex; align-items: center; gap: 8px; font-size: 14px; font-weight: 500; text-align: left; width: 100%; border: none; background: none; cursor: pointer; transition: 0.2s; border-bottom: 1px solid #f1f5f9; }
        .dropdown-content button:last-child { border-bottom: none; }
        .dropdown-content button:hover { background-color: #f8fafc; color: #0f172a; padding-left: 20px; }
    ...
    <main class="main-content">
        <header class="topbar">
            ...
            <div style="display: flex; gap: 12px;">
                ...
                <div class="content">
            <?php
            $view_scope = isset($_GET['view_scope']) ? $_GET['view_scope'] : 'team';
            $gl_emp_id = $_SESSION['emp_id'];
            $gl_filter_sql = ($view_scope === 'all') ? "" : " AND group_leader_id = '$gl_emp_id' ";

            $stats_query = mysqli_query($conn, "SELECT employment_type, COUNT(*) as count FROM employees WHERE role IN ('employee', 'qa', 'intern') $gl_filter_sql GROUP BY employment_type");
            ...
            ?>
            <!-- DASHBOARD FILTERS -->
            <details class="bg-white view-options-card" style='border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 20px; box-shadow: 0 2px 4px -1px rgba(0,0,0,0.03); width: max-content;'>
                ...
                <div style="padding: 8px 12px 12px 12px; border-top: 1px solid #e2e8f0; margin-top: 4px;">
                    <div style='display: flex; gap: 15px; flex-wrap: wrap;'>
                        <div style="font-size: 12px; font-weight: 600; color: #64748b; width: 100%;">Scope:</div>
                        <label style='display: flex; align-items: center; gap: 8px; font-size: 12px; font-weight: 600; color: #475569; cursor: pointer;'>
                            <input type='radio' name='view_scope' value='team' <?php echo $view_scope !== 'all' ? 'checked' : ''; ?> onchange='window.location.href="?view_scope=team"'> My Team Overview
                        </label>
                        <label style='display: flex; align-items: center; gap: 8px; font-size: 12px; font-weight: 600; color: #475569; cursor: pointer;'>
                            <input type='radio' name='view_scope' value='all' <?php echo $view_scope === 'all' ? 'checked' : ''; ?> onchange='window.location.href="?view_scope=all"'> Whole Employee Overview
                        </label>
                    </div>
                </div>
            </details>
            ...
            <div class="table-container">
                ...
                <form id="bulkDeleteForm" method="POST" action="">
                    ...
                    <table>
                        ...
                        $query = "SELECT * FROM employees WHERE role IN ('employee', 'qa', 'intern') $gl_filter_sql";
                        $where_clauses = [];
                        ...
                        if (mysqli_num_rows($result) > 0) {
                                ...
                                echo "<td class='actions-cell'>
                                    <div class='dropdown'>
                                        ...
                                        <div class='dropdown-content'>
                                            ...
                                            <button type='button' onclick='openEditModal(" . $row['id'] . ", \"" . htmlspecialchars($row['emp_id'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['welabel_id'] ?? '', ENT_QUOTES) . "\", \"" . htmlspecialchars(addslashes($row['name'])) . "\", \"" . htmlspecialchars($row['employment_type'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['role'], ENT_QUOTES) . "\", \"" . htmlspecialchars($row['lunch_window'], ENT_QUOTES) . "\")'>✏️ Edit</button>
                            ...
                            echo "<tr><td colspan='8'><div class='empty-state'><div class='empty-state-icon'>📋</div><div class='empty-state-text'>No employees found.</div></div></td></tr>";
                        }
                        ?>
                        </tbody>
                    </table>
